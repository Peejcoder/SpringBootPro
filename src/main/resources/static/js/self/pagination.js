...
var Pagination = {
    ...
    showPageCount: 7,  //显示页数的按钮格子 

    init: function (obj, callback,topFlag) {  //初始化
        this.bindListener(obj, callback,topFlag);
    },
    createHtml: function (pageIndex, recordCount, pageSize) { // pageIndex 当前页数 recordCount 是当前总数据的条数

        var pageCount = Math.ceil(recordCount / pageSize);  //总页数
        var showPageCount = this.showPageCount; //当前显示页数的按钮格子 
        var MaxCount = 10000000000;
        var HalfPageCount = (showPageCount + 1) / 2; //显示格子按钮的一半
        var html = [];
